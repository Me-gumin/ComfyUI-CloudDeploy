    ...
    with open(config_path, "r", encoding="utf-8") as f:
        ...
        if SETTINGS.keys():
            ...
            if source_lang not in LANGUAGES_CODES["source"]:
                SETTINGS.update({"source_lang": "English"})
                reset_lang = True

            if target_lang not in LANGUAGES_CODES["target"]:
                SETTINGS.update({"target_lang": "Chinese (simplified):"})
                reset_lang = True
...
DEEPLX_SERVER_URL = "http://127.0.0.1:41188/"
DEEPLX_SERVER_URL_TRANSLATE = "http://127.0.0.1:41188/translate"
